...
class FlowPredictor(nn.Module):
    ...
    def __init__(self, cfg):
        super().__init__()
        self.default_dt = cfg.integration_tau
        self.net = _build_mlp(cfg.embed_dim + 1, cfg.embed_dim,
                              cfg.hidden_dim, cfg.num_layers)

    def forward(self, y, dt=None):
        # y: (batch, d)
        # dt: float or tensor of shape (batch,) or (batch, 1)
        # Older visualization helpers omit dt; preserve their fixed-step
        # behavior while the pipeline passes dt explicitly.
        if dt is None:
            dt = self.default_dt
...
class _LegacyExtrusionEncoder(nn.Module):
    """Pre-public-branch encoder retained for archived checkpoints."""
    ...
    def __init__(self, cfg):
        ...
        self.net = _build_mlp(
            self.d + 1, self.e, cfg.hidden_dim, cfg.num_layers
        )
    ...
    def forward(self, x):
        ...
        y = torch.zeros(
            x.shape[0], self.e, device=x.device, dtype=x.dtype
        )
        if base_mask.any():
        ...
        if (~base_mask).any():
            ...
            base_point = torch.zeros(
                x_cyl.shape[0], self.e, device=x.device, dtype=x.dtype
            )
...
class _LegacyFlowPredictor(nn.Module):
    """Fixed-step archived flow; ``dt`` is accepted but has no effect."""

    def __init__(self, cfg):
        super().__init__()
        self.net = _build_mlp(
            cfg.embed_dim, cfg.embed_dim, cfg.hidden_dim, cfg.num_layers
        )
...
class _LegacyStabilizationDecoder(nn.Module):
    """Pre-public-branch decoder retained for archived checkpoints."""
    ...
    def __init__(self, cfg):
        ...
        self.e = cfg.embed_dim
        self.net = _build_mlp(
            self.e, self.d + 1, cfg.hidden_dim, cfg.num_layers
        )

    def forward(self, y):
        x_base = y[:, : self.d + 1]
        x_recon = x_base + 0.1 * self.net(y)
        x_recon[:, self.d : self.d + 1] = 0
        return x_recon
...
class SuspensionNetworks(nn.Module):
    def __init__(self, cfg=config):
        super().__init__()
        self.cfg = cfg
        self.checkpoint_api = "public-section4"
        self._set_architecture(legacy=False)

    def _set_architecture(self, legacy):
        reference = next(self.parameters(), None)
        if legacy:
            modules = (
                _LegacyExtrusionEncoder(self.cfg),
                _LegacyFlowPredictor(self.cfg),
                _LegacyStabilizationDecoder(self.cfg),
            )
            self.checkpoint_api = "legacy-main"
        else:
            modules = (
                Encoder(self.cfg),
                FlowPredictor(self.cfg),
                Decoder(self.cfg),
            )
            self.checkpoint_api = "public-section4"
        ...
        self.E, self.F, self.D = modules

    def load_state_dict(self, state_dict, strict=True, assign=False):
        """Select the matching network semantics before loading a checkpoint."""
        key = "F.net.0.weight"
        weight = state_dict.get(key)
        if weight is not None:
            legacy = weight.shape[1] == self.cfg.embed_dim
            current = weight.shape[1] == self.cfg.embed_dim + 1
            if not (legacy or current):
                raise RuntimeError(
                    f"unsupported {key} input width {weight.shape[1]} for "
                    f"embed_dim={self.cfg.embed_dim}"
                )
            expected_api = "legacy-main" if legacy else "public-section4"
            if self.checkpoint_api != expected_api:
                self._set_architecture(legacy=legacy)
