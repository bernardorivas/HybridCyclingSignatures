...
\begin{frame}{The Regularity Loss Terms}
  ...
  \medskip
  \textbf{Seam velocity.}\quad With \(z_k = E_\theta(y_k)\) and step \(\tau\),
  \[ L_v = \frac{1}{|\mathcal{S}|}\sum_{k\in\mathcal{S}}
     \Big\lVert \frac{z_k - z_{k-1}}{\tau}
       - \frac{z_{k+2} - z_{k+1}}{\tau}\Big\rVert^2 . \]
  \(L_v = 0\) matches the latent velocity just before and just after a seam,
  so the latent curve is \(C^1\) across it.
