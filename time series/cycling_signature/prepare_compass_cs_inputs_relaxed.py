"""
Prepare cycling-signature pipeline inputs using the learned relaxed-space
encoder E_theta.

Encodes the canonical 5-impact compass rollout through the trained encoder
from runs/compass_gait/model.pt, writes canonical artifacts with the
`_relaxed` suffix, and reports encoder-quality diagnostics (reconstruction,
injectivity, tangent stability, junction angles).

The encoder is the generic Section-4 encoder E : R^(n+1) -> R^d, a residual
MLP initialised near the identity. It embeds the relaxed space
X' = X union (G x [0,1]), not the hybrifold; this distinguishes us from the
original CHyLL of [Teng et al., 2024] which targets the hybrifold directly.
See pipelines/compass_gait_relaxed/SPEC.md for the full architecture spec.

Usage:
    python "time series/cycling_signature/prepare_compass_cs_inputs_relaxed.py"

Canonical outputs under data/compass_gait/:
...
parser.add_argument(
    "--model",
    default=os.path.join(REPO_ROOT, "runs", "compass_gait", "model.pt"),
)
...
n_s = args.n_s
s_vals = np.linspace(0.0, 1.0, n_s + 2)[1:-1]  # interior only, avoids duplicates
...
assert np.allclose(tx_norms, 1.0, atol=1e-6), f"tangent unit-norm check failed: {tx_norms.min()}..{tx_norms.max()}"
...
_log("")
_log("=" * 60)
_log("PHASE A  |  RELAXED-SPACE ENCODER DIAGNOSTICS (compass)")
_log("=" * 60)
...
# Full reconstruction error (includes the decoder's s->0 forcing, which is
# irreducible for bridge samples by design).
full_err = np.linalg.norm(x_rec - X_in, axis=1)
# State-only error: meaningful for bridges since decoder forces s=0.
state_err = np.linalg.norm(x_rec[:, :config.state_dim] - X_in[:, :config.state_dim], axis=1)

def _row(lbl, arr):
    _log(f"  {lbl:<24}  mean={arr.mean():.3e}  median={np.median(arr):.3e}  p95={np.percentile(arr, 95):.3e}  max={arr.max():.3e}")
...
_log("  note: the decoder now reconstructs s freely (no forcing); bridge")
...
N = len(Z)
# Exclude samples within 20 indices (within same arc / adjacent bridge region).
time_gap = 20
...
# Collapse signal: latent distance << input distance for time-distant neighbors.
# Ratio close to 1 = consistent metric; much less than 1 = latent collapsed.
...
_log("  note: 5 bridges here are 5 observations of ONE canonical jump;")
_log("        low inter-bridge distance is expected and topologically correct.")
